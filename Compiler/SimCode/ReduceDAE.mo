...
algorithm
  ...
  matchcontinue (inEquationLst,inModelInfo,reduceList,inArgs)
    ...
    case (eqns,SimCode.MODELINFO(name,description,directory,
      ...
      vars,functions,labels,nClocks,nSubClocks,hasLargeLinearEquationSystems),_,Absyn.FUNCTIONARGS(args = {Absyn.CREF(_), Absyn.ARRAY(arrayExp = exp_list)}))
      equation
      ...
      then
        (eqns_1,SimCode.MODELINFO(name,description,directory,varInfo_1,vars_1,functions,labels_2,nClocks,nSubClocks,hasLargeLinearEquationSystems));
    ...
    case (eqns,SimCode.MODELINFO(name,description,directory,
      SimCode.VARINFO(nZC,nTE,nR,nMEF,nStates,nAlg,nDiscReal,nIntAlg,nBoolAlg,nAlgAlias,nIntAlias,nBoolAlias,
              nParam,nIntParam,nBoolParam,nOut,nIn,nExtObj,nStringAlg,nStringParam,nStringAlias,nEq,nLSys,nNLSys,nMixSys,nStateSet,nJacobian,nOptCons,
			  nOptFinalConst,nSensParam),
      vars,functions,labels,nClocks,nSubClocks,hasLargeLinearEquationSystems),_,args)
      equation
    ...
    case (SimCodeVar.SIMVARS(states,derVar,alg,disAlg,intAlg,boolAlg,inVar,outVar,algAlias,intAlias,boolAlias,param,
      ...
      equation
        ...
        simVar_1 = SimCodeVar.SIMVAR(DAE.CREF_IDENT(name1,DAE.T_REAL_DEFAULT,{}),BackendDAE.PARAM(),"","","",p,NONE(),NONE(),SOME(DAE.RCONST(1.0)),NONE(),
                   true,DAE.T_REAL_DEFAULT,false,NONE(),SimCodeVar.NOALIAS(),DAE.emptyElementSource,SimCodeVar.INTERNAL(),NONE(),{},false,false,false,NONE());
